...
var sidebar = require("sdk/ui/sidebar").Sidebar({
    ...
    onAttach: function (worker) {
        ...
        worker.port.on('pushTime', function(task){
            var nowTime = new Date().getTime();
            var startTime = task.startAt;
            var progressTime = (nowTime - startTime)/1000/3600;

            var requestPushTime = Request({
                url: "http://" + ss.storage.redmineConfig.redmineHost + '/time_entries.json?key=' + ss.storage.redmineConfig.redmineKey,
                content: "time_entry[issue_id]=" + task.id + "&time_entry[hours]=" + progressTime + '&time_entry[activity_id]=9',
                onComplete: function(response){
                    worker.port.emit('timePushed');
                }
            }).post();
